...
public class UpgradeUIController : MonoBehaviour
{
    ...
    public void ShowRandomWeapons(PlayerController player,int lev)
    {
        PlayerController = player;
        if (allWeapons == null || allWeapons.Length == 0)
        {
        ...
        foreach (Weapons weapon in allWeapons)
        {
            if (weapon != null && !PlayerController.weaponUpgradeLevels.ContainsKey(weapon))
            {
                bool isBat = weapon.weaponType == Weapons.WeaponType.Bat;
                bool isShotgun = weapon.weaponType == Weapons.WeaponType.Shotgun;

                Debug.Log(isBat);

                if (lev == 1 && (isBat || isShotgun))
                {
